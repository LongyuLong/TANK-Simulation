...
## SA 계약 (런타임 접촉) — planning.py
역할: **글로벌 = 재계획/평가 서비스**, 결심·실행은 로컬/사격팀(아래).
```python
# 런타임 접촉 시 (로컬이 호출)
path, st = planning.replan_on_contact(cur_pos, goal, known_enemies,
                                      extra_obstacles=[(r,c),...])  # 적/장애물 갱신 후 재계획
a = planning.assess_contact(cur_pos, goal, known_enemies, new_enemy)
# a = {self_risk, can_engage, bypass_cost, blocks_goal, bypass_path}
d = planning.decide_engagement(a, policy="balanced")   # 참고 stub (로컬/web 소유)
#   "engage" → 사격팀/로컬(사격위치+사격) / "bypass" → a["bypass_path"]로 주행
```
- `replan_on_contact` = `plan_threat` 얇은 래퍼(현재위치 start + known-set/장애물 갱신). **D\* 도입 시 내부만 교체**, 계약 불변.
- `assess_contact` = 기존 threat/is_visible 재사용(신규 상태 없음).
- `decide_engagement` = **교체용 stub** — 실제 결심은 로컬 런타임/ web 운용자.
- 시나리오: S1(적)=assess→decide→engage|bypass / S3(장애물)=결심없이 replan_on_contact / S4′(화력지원)=새 goal로 replan.
